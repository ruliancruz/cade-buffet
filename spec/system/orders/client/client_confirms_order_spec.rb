...
describe 'Client confirms order' do
  it "with success if the order hasn't expired yet" do
    ...
    expect(page).not_to have_content 'O pedido expirou!'
  ...
  it "and doesn't find confirm button if the order has expired" do
    buffet_owner = BuffetOwner
      .create! email: 'buffet_owner@example.com', password: 'password'
    ...
    order = Order
      .new date: I18n.localize(Date.current + 2.week),
           ...
           details: 'Quero que inclua queijo suíço e vinho tinto.',
           address: buffet.full_address,
           expiration_date: I18n.localize(Date.current),
           base_price: base_price,
           price_adjustment: -500,
           price_adjustment_description: 'Promoção especial!',
           status: :approved_by_buffet,
    ...
    expect(page).to have_content 'O pedido expirou!'
    expect(page).not_to have_button 'Confirmar Pedido'
  end
end